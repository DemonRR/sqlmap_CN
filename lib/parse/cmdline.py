...
def cmdLineParser(argv=None):
    ...
    try:
        ...
        parser.add_argument("-v", dest="verbose", type=int,
            help="冗长级别: 0-6 (默认 %d)" % defaults.verbose)
        ...
        target.add_argument("-l", dest="logFile",
            help="从Burp或WebScarab代理日志文件解析目标(s)")
        ...
        request.add_argument("-H", "--header", dest="header",
            help="额外的标头 (例如 \"X-Forwarded-For: 127.0.0.1\")")
        ...
        request.add_argument("--mobile", dest="mobile", action="store_true",
            help="通过 HTTP User-Agent 标头模拟智能手机")
        ...
        request.add_argument("--headers", dest="headers",
            help="额外的标头 (e.g. \"Accept-Language: fr\\nETag: 123\")")
        ...
        # Injection options
        injection = parser.add_argument_group("注入", "这些选项可用于指定要测试的参数、提供自定义注入有效载荷和可选的篡改脚本")
