...
def vulnTest():
    ...
    if not vulnserver._alive:
        logger.error("vulnserver 实例化中出现问题 (地址: 'http://%s:%s')" % (address, port))
        return False
    else:
        logger.info("vulnserver 正在运行 'http://%s:%s'..." % (address, port))
    ...
    clearConsoleLine()
    if retVal:
        logger.info("vuln 测试最终结果： 通过")
    else:
        logger.error("vuln 测试最终结果： 失败")
