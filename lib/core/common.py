                        ...
                        if match:
                            try:
                                ...
                                if candidates:
                                    ...
                                    message += "是JSON可反序列化的。你想在里面注入吗？ [y/N] "
                            ...
                            message = "似乎为 %s 参数 '%s'提供了值 " % ("%s " % place if place != parameter else "", parameter)
                            message += "有边界，你想在里面注入吗？('%s') [y/N] " % getUnicode(_)
        ...
        if dbmsName == conf.dbms or conf.dbms.lower() in data[0]:
            try:
                ...
                if dbmsName in (DBMS.ACCESS, DBMS.SQLITE, DBMS.FIREBIRD):
                    if remote:
                        warnMsg = "不支持"
                        warnMsg += "%s DBMS的网络直接连接" % dbmsName
                        logger.warning(warnMsg)
...
def getLocalIP():
    ...
    try:
        ...
        debugMsg = "打开通往"
        debugMsg += "'%s'的套接字连接时出错" % conf.hostname
        logger.debug(debugMsg)
        ...
        with kb.locks.cache:
            if filename not in kb.cache.content:
                ...
                try:
                    ...
                    errMsg = "试图读取文件"
                    errMsg += "'%s' 的内容时出错了('%s')" % (filename, getSafeExString(ex))
                    raise SqlmapSystemException(errMsg)
...
def getFileItems(filename, commentPrefix='#', unicoded=True, lowercase=False, unique=False):
    ...
    try:
        ...
        errMsg = "尝试读取文件"
        errMsg += "'%s' ('%s')时出错了" % (filename, getSafeExString(ex))
        raise SqlmapSystemException(errMsg)
