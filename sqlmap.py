...
try:
    ...
    try:
        ...
        sys.exit("[!] 检测到安装错误（模块丢失）。详情请访问 'https://github.com/sqlmapproject/sqlmap/#installation' for further details")
    ...
    if "logger" in globals():
        ...
        import time
        sys.exit("\r[%s] [严重的] %s" % (time.strftime("%X"), errMsg))
...
def checkEnvironment():
    try:
        ...
        errMsg += "请将sqlmap的目录移至其他位置"
        logger.critical(errMsg)
        raise SystemExit

    if LooseVersion(VERSION) < LooseVersion("1.0"):
        errMsg = "您的运行时环境（例如PYTHONPATH）"
        errMsg += "已损坏。请确保您没有使用旧版本的运行时脚本运行"
        errMsg += "较新版本的sqlmap"
...
def main():
    ...
    try:
                        ...
                        if conf.crawlDepth and conf.bulkFile:
                            ...
                            for i in xrange(len(targets)):
                                ...
                                try:
                                    ...
                                    infoMsg = "开始为目标URL '%s' 进行爬取 (%d/%d)" % (target, i + 1, len(targets))
                                    logger.info(infoMsg)
        ...
        if any(_ in excMsg for _ in ("MemoryError", "Cannot allocate memory")):
            ...
            errMsg = "无效的URL（违反双向IDNA规则 - RFC 5893）"
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = "Python解释器出现错误，"
            errMsg += "已在2.7版本修复。请相应地更新"
            ...
            errMsg = "当sqlmap与Python 3.11的ALPHA版本一起运行时，存在已知问题。"
            ...
            errMsg += " (参考: 'https://github.com/sqlmapproject/sqlmap/issues/4572#issuecomment-77504"
            logger.critical(errMsg)
            raise SystemExit

        elif all(_ in excMsg for _ in ("window = tkinter.Tk()",)):
            errMsg = "在初始化 GUI 界面时发生问题 "
            errMsg += "('%s')" % excMsg.strip().split('\n')[-1]
            logger.critical(errMsg)
            raise SystemExit

        elif any(_ in excMsg for _ in ("unable to access item 'liveTest'",)):
            errMsg = "检测到使用不同版本 sqlmap 的文件"
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = "代码校验和失败（关闭自动创建问题功能）。"
            errMsg += "你应该从官方 GitHub 仓库获取最新的开发版本，网址为 '%s'" % GIT_PAGE
            ...
            errMsg = "运行多进程时存在权限问题。"
            errMsg += "请使用 '--disable-multi' 参数重新运行"
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = "枚举时发生问题。"
            errMsg += "由于存在较大可能的误报情况，"
            errMsg += "建议使用 '--flush-session' 参数重新运行"
            logger.critical(errMsg)
            raise SystemExit

        elif "database disk image is malformed" in excMsg:
            errMsg = "本地会话文件似乎已损坏。请使用 '--flush-session' 参数重新运行"
            logger.critical(errMsg)
            raise SystemExit

        elif "AttributeError: 'module' object has no attribute 'F_GETFD'" in excMsg:
            errMsg = "无效的运行时 (\"%s\") " % excMsg.split("Error: ")[-1].strip()
            ...
            errMsg = "你的某个 .pyc 文件已损坏%s" % (" ('%s')" % match.group(1) if match else "")
            errMsg += "。请删除系统中的 .pyc 文件以修复问题"
            logger.critical(errMsg)
            raise SystemExit
